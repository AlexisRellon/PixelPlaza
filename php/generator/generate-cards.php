...
// Fetch the Browse Products
$products = fetchBrowseProducts($conn);

?>

<?php foreach ($products as $product): ?>
    <?php
    $trimURL = '.' . ltrim($product['ImageURL'], '.');
    ...
    ?>
    <a href="./collections/product-page.php?productID=<?php echo $product['ProductID'] ?>" class="card p-5 flex flex-column justify-start">
        <!-- Hover toolbar -->
        ...
        <img src="<?php echo $trimURL ?>" alt="" loading="lazy" class="product-image">
        <div class="product-details w-100 text-center">
            <p class="brand font-bold text-sm"><?php echo $brandName ?></p>
            <p class="product font-extrabold text-2xl"><?php echo $product['Name']; ?></p>
            <p class="price text-base">₱ <?php echo $product['Price']; ?></p>
        </div>
    </a>
<?php endforeach; ?>
